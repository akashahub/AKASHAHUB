...
async function joinLive(roomId) {
  if (!window.LivekitClient) {
    toast("LiveKit ainda carregando");
    return;
  }
  try {
    ...
    lkRoom.on(RoomEvent.TrackSubscribed, (track, pub, participant) => {
      attachLiveTrack(track, participant.identity);
    });
    lkRoom.on(RoomEvent.TrackUnsubscribed, (track) => {
      track.detach().forEach((el) => el.remove());
    });
    lkRoom.on(RoomEvent.ParticipantDisconnected, (p) => {
      const box = document.getElementById("lk-" + p.identity);
      if (box) box.remove();
    });
...
function restoreLiveTiles() {
  ...
  lkRoom.localParticipant.trackPublications.forEach((pub) => {
    if (pub.track) attachLiveTrack(pub.track, me.uid, true);
  });
  lkRoom.remoteParticipants.forEach((p) => {
    p.trackPublications.forEach((pub) => {
      if (pub.track) attachLiveTrack(pub.track, p.identity);
    });
  });
}
    ...
    ${LK_ROOMS.map((r) => `
      <div class="card">
        <div class="row">
          <div>
            <h3>${r.title}</h3>
            <p class="m">${r.id}${lkCurrent === r.id ? " · conectado" : ""}</p>
          </div>
          ${lkCurrent === r.id
            ? `<button class="btn danger" style="margin:0;width:auto" onclick="leaveLive()">Sair</button>`
            : `<button class="btn btn3" style="margin:0;width:auto" onclick="joinLive('${r.id}')">Entrar</button>`}
        </div>
      </div>`).join("")}
    <div class="card">
      <div class="m">${connected ? "Sala " + lkCurrent : "Nenhuma sala aberta neste aparelho"}</div>
      <div id="lk-grid" class="lk-grid"></div>
    </div>

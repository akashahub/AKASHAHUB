...
export function buildNav() {
  ...
  nav.innerHTML = items
    .map(
      (i) =>
        `<a class="nav-item${currentView === i.id || (currentView.startsWith("module:") && i.id === "modules") ? " active" : ""}" href="#" data-view="${i.id}"><span class="ico">${i.ico}</span>${esc(i.label)}</a>`
    )
    .join("");
...
export function navigate(view) {
  ...
  if (top) top.textContent = titles[view] || (view.startsWith("module:") ? "Módulo" : "AF");

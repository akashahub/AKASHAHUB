...
async function openCashflow() {
  ...
  openFloat(
    ...
    <div class="cash-row">
      ...
      <select id="cCat"><option>Moradia</option><option>Alimentação</option><option>Transporte</option><option>Assinaturas</option><option>Receita</option><option>Investimento</option><option>Outro</option></select>
    </div>
    <div class="cash-row">
      <select id="cAct"><option>manter</option><option>reduzir</option><option>cancelar</option></select>
      <select id="cRec"><option value="nao">Avulso</option><option value="sim">Recorrente</option></select>
    </div>
    <button class="tool-btn" type="button" id="btnAddCash">+ Lançar</button>
    <div class="cash-kpis" id="cashKpi"></div>
    <div class="cash-list" id="cashList"></div>`,
    `<button class="btn btn-inline" type="button" id="btnSaveCash">Salvar auditoria</button>`
  );
  const signed = (it) => {
    const n = Number(it.val || 0);
    return (it.tipo || (it.cat === "Receita" ? "entrada" : "saida")) === "entrada" ? n : -n;
  };
  const render = () => {
    ...
    el.innerHTML =
      window._cash
        .map((it, i) => {
          const s = signed(it);
          const sign = s >= 0 ? "+" : "−";
          return `<div class="cash-item"><span>${esc(it.desc)} · ${esc(it.cat)} · ${sign} R$ ${Math.abs(Number(it.val)).toFixed(2)}</span>
    ...
    const ent = window._cash.filter((i) => signed(i) > 0).reduce((s, i) => s + signed(i), 0);
    const sai = window._cash.filter((i) => signed(i) < 0).reduce((s, i) => s + signed(i), 0);
    const kpi = document.getElementById("cashKpi");
    if (kpi) kpi.innerHTML = `<div>Entradas <b>R$ ${ent.toFixed(2)}</b></div><div>Saídas <b>R$ ${Math.abs(sai).toFixed(2)}</b></div><div>Saldo <b>R$ ${(ent + sai).toFixed(2)}</b></div>`;
  };
  render();
...
async function openNetwork() {
  ...
  openFloat(
    "Contatos · Rede",
    `<div class="cash-row"><input id="nName" placeholder="Nome"><input id="nRole" placeholder="Papel / empresa"></div>
     <div class="cash-row"><input id="nChan" placeholder="Canal (wpp, ig, e-mail)"><input id="nNext" placeholder="Próxima entrega"></div>
     <button class="tool-btn" type="button" id="btnAddNet">+ Contato</button>
     <div class="cash-list" id="netList"></div>`,
    `<button class="btn btn-inline" type="button" id="btnSaveNet">Salvar rede</button>`
  );
  const render = () => {
    ...
    el.innerHTML =
      window._net
        .map(
          (c, i) =>
            `<div class="cash-item"><span><b>${esc(c.name)}</b> · ${esc(c.role || "—")}<br><small>${esc(c.chan || "")} · ${esc(c.next || "")}</small></span>
        <button type="button" data-rm="${i}">remover</button></div>`
        )
        .join("") || "<p class='empty'>Nenhum contato ainda</p>";
    el.querySelectorAll("[data-rm]").forEach((b) => {
      b.onclick = () => {
  ...
  document.getElementById("btnAddNet").onclick = () => {
    ...
    window._net.push({
      name,
      role: document.getElementById("nRole").value.trim(),
      chan: document.getElementById("nChan").value.trim(),
      next: document.getElementById("nNext").value.trim()
    });
    document.getElementById("nName").value = "";
    document.getElementById("nRole").value = "";
    document.getElementById("nChan").value = "";

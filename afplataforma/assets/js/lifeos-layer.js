...
function renderHabits(list) {
  return list
    .map(
      ...
      <label><input type="checkbox" ${h.on ? "checked" : ""} data-act="afToggleHabito" data-i="${i}"> <span>${esc(h.name)}</span></label>
...
window.afToggleHabito = (el) => {
  ...
  window._habits[i].on = el.checked;
...
export function viewMindZone() {
  return `<div class="view active">${back()}
    ...
    <p class="hero-sub">Largura de banda mental antes de decisão financeira.</p>
    <div class="mat-grid">
      <div class="mat-card"><h4>Respiração 4-7-8</h4><p>Inspira 4 · segura 7 · solta 8. Três ciclos.</p>
        <button class="tool-btn" type="button" data-act="afBreath478">Iniciar</button>
        <p class="notes-meta" id="breathMsg">—</p></div>
      <div class="mat-card"><h4>Box 4×4</h4><p>Quadrado: inspira · segura · expira · segura.</p>
        <button class="tool-btn" type="button" data-act="afBreathBox">Iniciar</button></div>
      <div class="mat-card"><h4>Jogo · Foco 60s</h4><p>Toque o ouro. Treina atenção seletiva.</p>
        <button class="tool-btn" type="button" data-act="afFocusGame">Jogar</button>
        <div id="focusGame" class="focus-board"></div></div>
    </div>
    <p class="notes-hint">Áudios guiados: camada pronta — você cola os arquivos em assets/audio/ depois.</p>
  </div>`;
}
window.afBreath478 = () => {
  const el = document.getElementById("breathMsg");
  const steps = ["Inspira 4…", "Segura 7…", "Solta 8…", "Ciclo 2", "Ciclo 3", "Concluído."];
  let i = 0;
  el.textContent = steps[0];
  const id = setInterval(() => {
    i++;
    if (!steps[i]) return clearInterval(id);
    el.textContent = steps[i];
  }, 4000);
};
window.afBreathBox = () => toast("Box: 4s cada lado · 4 voltas. Comece agora.");
window.afFocusGame = () => {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    b.type = "button";
    b.className = "focus-dot";
    if (i === Math.floor(Math.random() * 12)) b.classList.add("gold");
    b.onclick = () => {
      if (b.classList.contains("gold")) {
        score++;
        toast("Acerto · " + score);
        window.afFocusGame();
      }
...
export function viewProdutividade() {
  return `<div class="view active">${back()}
    ...
    <p class="hero-sub">Pomodoro alinhado ao bloco de execução do módulo 3.</p>

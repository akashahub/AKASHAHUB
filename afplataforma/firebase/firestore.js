...
/* ── Presença e salas de call ── */
export async function upsertPresence(uid, data) {
  if (!uid) return;
  await setDoc(doc(db, "afPresence", uid), { ...data, lastSeen: serverTimestamp() }, { merge: true });
}
...
export async function getAfRoom(roomId) {
  const id = roomId || "mentoria-principal";
  const snap = await getDoc(doc(db, "afRooms", id));
  if (!snap.exists()) {
    return { id, title: "Mentoria principal", maxParticipants: 12, locked: false, ended: false, kicked: [], muted: [] };
  }
  return { id, ...snap.data() };
}

export function listenAfRoom(roomId, cb) {
  const id = roomId || "mentoria-principal";
  return onSnapshot(doc(db, "afRooms", id), (snap) => {
    if (!snap.exists()) cb({ id, title: "Mentoria principal", maxParticipants: 12, locked: false, ended: false, kicked: [], muted: [] });
    else cb({ id, ...snap.data() });
  });
}

export async function ensureAfRoom(roomId, extra) {
  const id = roomId || "mentoria-principal";
  const cur = await getDoc(doc(db, "afRooms", id));
  if (!cur.exists()) {
    await setDoc(doc(db, "afRooms", id), {
      title: extra?.title || "Mentoria principal",
      maxParticipants: extra?.maxParticipants || 12,
      locked: false,
      ended: false,
      kicked: [],
      muted: [],
...
export async function updateAfRoom(roomId, data) {
  const id = roomId || "mentoria-principal";
  await setDoc(doc(db, "afRooms", id), { ...data, updatedAt: serverTimestamp() }, { merge: true });
}

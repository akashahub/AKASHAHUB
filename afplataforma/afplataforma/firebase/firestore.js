...
export async function getAfTool(uid, toolId) {
  ...
  const snap = await getDoc(doc(db, "afToolData", uid, "tools", toolId));
  return snap.exists() ? snap.data() : null;
}
...
export async function saveAfTool(uid, toolId, data) {
  ...
  await setDoc(
    doc(db, "afToolData", uid, "tools", toolId),
...
export async function getMenteeNotes(uid, callId = "default") {
  if (!uid) return null;
  const snap = await getDoc(doc(db, "afCallNotes", uid, "notes", callId));
  return snap.exists() ? snap.data() : null;
}
...
export async function saveMenteeNotes(uid, callId = "default", content = "") {
  ...
  await setDoc(
    doc(db, "afCallNotes", uid, "notes", callId),
...
export async function getMentorPrivateNotes(menteeUid, callId = "default") {
  ...
  const snap = await getDoc(doc(db, "afMentorPrivateNotes", menteeUid, "notes", callId));
  return snap.exists() ? snap.data() : null;
}
...
export async function saveMentorPrivateNotes(menteeUid, callId = "default", content = "") {
  ...
  await setDoc(
    doc(db, "afMentorPrivateNotes", menteeUid, "notes", callId),
    { content, updatedAt: serverTimestamp() },
    { merge: true }
  );
}
